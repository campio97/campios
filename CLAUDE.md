...
CampiOS is a **custom immutable Linux desktop OS image**, not an application. The "build" produces an OCI container image that is itself a bootable operating system (a [bootc](https://github.com/bootc-dev/bootc) image). It derives from the Universal Blue [`image-template`](https://github.com/ublue-os/image-template) and is based on `quay.io/rakuos/rakuos-base-nvidia` (a Fedora Atomic / ublue NVIDIA base). rakuos migrated its registry ghcr.io → GitLab → **quay.io** with the Origami Linux merge (release 44, 2026-06-02); the old `ghcr.io/rakuos/rakuos-base-nvidia` is frozen at Fedora 43 (last tag 2026-04-19), while quay.io is rebuilt daily on Fedora 44.
...
`nvidia-container-toolkit` is a **special install, inline in `build.sh`, not `packages/install.txt`** — for two reasons. First, it lives only in the `terra-nvidia` repo, which **the base does not configure**: `rakuos-base-nvidia` ships `terra-release` (repo `terra`) but *not* `terra-release-nvidia`, because it bakes the NVIDIA driver in at its own build time and no longer needs the repo. `build.sh` therefore installs `terra-release-nvidia`, installs the toolkit, then **removes the release package again** so the final image doesn't carry a repo offering NVIDIA drivers newer than the base's — the same userspace-vs-kernel-module drift documented above for `nvidia-driver-cuda`. Second, `install.txt` is a *single* dnf transaction for every CampiOS package: an unavailable third-party repo in there fails the whole image build, not just one package. The install is wrapped in an `if` and only warns on failure — without the toolkit the image still boots, containers just don't see the GPU.

The toolkit only provides `nvidia-ctk` — **the Terra package ships neither the upstream `nvidia-cdi-refresh.{service,path}` units nor an actual CDI spec** (`/etc/cdi/nvidia.yaml` is a `%ghost`: the path is reserved, the file is never generated). So CampiOS generates the spec itself.
...
- The container signature policy (`build_files/etc/containers/policy.json`) defaults to `reject` and only trusts: `ghcr.io/campio97/campios` (publish target), `quay.io` (host-wide — covers the current OS base `quay.io/rakuos/rakuos-base-nvidia` **and** the user's own `distrobox` boxes), `registry.fedoraproject.org`, `docker.io`, plus the now-legacy rakuos sources `ghcr.io/rakuos` and the rakuos GitLab registry (both superseded by quay.io — kept harmlessly, prune when convenient) — update it if the OS base, publish target, or a distrobox base from a new registry changes. **Any rootless `podman`/`distrobox` pull is governed by this policy too:** pulling from an untrusted registry fails with `Source image rejected: ... rejected by policy`.
