...
- The container signature policy (`build_files/etc/containers/policy.json`) only trusts `ghcr.io/rakuos`, `ghcr.io/campio97/campios`, and the rakuos GitLab registry — update it if the base image or publish target changes.
